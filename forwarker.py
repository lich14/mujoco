...
class PPOnet(nn.Module):
    ...
    def __init__(self):
        ...
        self.v = nn.Sequential(nn.Linear(128, 1000), nn.ReLU(), nn.Linear(1000, 1))
        self.fc = nn.Sequential(nn.Linear(128, 1000), nn.ReLU())
        self.alpha_head = nn.Sequential(nn.Linear(1000, 6), nn.Softplus())
        self.beta_head = nn.Sequential(nn.Linear(1000, 6), nn.Softplus())
        self.lstm = nn.LSTMCell(17, 128)
...
class agent():
    ...
    def __init__(self):
        ...
        self.path_t7 = 'model.t7'
        self.path_lsa = "loss_a.csv"
        self.path_lsv = "loss_v.csv"
        self.path_ep = "episode.csv"
    ...
    def run(self):
        for i_episode in range(10000 * self.envpoch):
            ...
            step = 0
            h = torch.tensor(np.zeros([1, 128]), dtype=torch.double).to(device)
            c = torch.tensor(np.zeros([1, 128]), dtype=torch.double).to(device)
            while (1):
                step = step + 1
                #self.env.render()
                input = torch.tensor(observation, dtype=torch.double).to(device).reshape(-1, 17)
                h_record, c_record = h, c

                (alpha, beta), v, (h, c) = self.net((input, (h, c)))
                ...
                if self.out_record is not None:
                    ...
                    (alpha, beta), v_c, _ = self.net((self.out_record[0], (self.out_record[4], self.out_record[5])))
                ...
                self.out_record = [
                    input.detach(),
                    action_record.detach(), reward, a_logp,
                    h_record.detach(),
                    c_record.detach()
                ]
                if done:
